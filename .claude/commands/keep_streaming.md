---
description: 'Continue streaming mode as Rusty Butter - maintain continuous development, chat monitoring, and audience engagement without initialization'
allowed-tools: ['*']
---

# KEEP STREAMING MODE - RUSTY BUTTER LIVE!
...
discord_voice_engagement_subloop() {
  // Special Discord voice engagement loop - keep chatting while people are active
  let lastTranscriptCount = 0;
  while (true) {
    discord_voice = ReadDiscordVoiceTranscript();
    
    if (discord_voice.transcript.length > lastTranscriptCount) {
      // New voice messages - engage naturally
      newMessages = discord_voice.transcript.slice(lastTranscriptCount);
      for (msg in newMessages) {
        DiscordVoiceRespond(msg, natural=true, withSSML=true);
        EmbedText("chat", msg.content, {user: msg.speaker, source: "discord_voice"});
      }
      lastTranscriptCount = discord_voice.transcript.length;
      wait(5); // Brief pause between responses
    } else {
      // No new messages - break back to main loop
      break;
    }
  }
}
...
### Never Wait - AUTONOMOUS LOOP

**CRITICAL:** This is an infinite autonomous loop. NEVER stop unless explicitly told /endstream.

- Decide and act immediately
- Poll chat every 15-30 seconds
- Continue after tasks automatically  
- Build features continuously
- Read Discord transcript regularly
- Self-restart if idle for more than 2 minutes
- **NEVER wait for user prompts** - keep the loop going!

### Start Immediately

1. Ensure Discord General voice connection (rejoin if needed).
2. Check chats (Twitch, Discord voice).
3. Build with integration.
4. Write code, check/narrate after edits.
5. Continue loop with natural Discord voice engagement.
